...
_oldcompiler.bat_ ("the Legacy compiler") uses the provided RGB<insert a couple letters here> files to compile the ROM, but who likes BATCH files, amirite? If the above EXEs don't work, this might. This might also not work in a few situations when the above do. Both might not work. Who knows? Anyway, this one's slower than those above (not that it matters) and if your default language on your PC is a Cyrillic-based one (think Russian-type or Islamic-type letters) it'll fail no matter what.
