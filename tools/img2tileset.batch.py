...
infilepaths=open("infilepaths.txt","r")
infilenames=infilepaths.readlines()
counter=0
print str(len(infilenames))+" files to load.\n"
for index in infilenames:
    infile=str(infilenames[counter]).rstrip()
    #[0:int(len(infilenames[counter]) - 1)]
    mktileset(["img2tileset",str(infile),str(infile[0:(len(infile)-4)]+".bin")])
    print"File "+str(counter+1)+" processed."
    counter+=1
